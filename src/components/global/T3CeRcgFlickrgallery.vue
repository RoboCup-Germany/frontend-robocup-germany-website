...
const dotScale = (index: number): number => {
  const d = Math.abs(index - currentIndex.value)
  if (d === 0) return 1
  if (d === 1) return 0.75
  if (d === 2) return 0.55
  return 0.45
      ...
      <div class="w-full">
        ...
        >
          ...
          <div v-else class="relative w-full max-h-[75vh]" :style="frameStyle">
            ...
            <div v-if="hasMultiplePhotos" class="pointer-events-none absolute inset-x-0 bottom-0 z-20">
              <div
                aria-hidden="true"
                role="presentation"
              ...
              >
                ...
                <TransitionGroup :name="dotTransitionName" tag="div" class="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center gap-2">
